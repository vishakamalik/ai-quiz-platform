  ...
  return (
    <div className="animate-fade-in max-w-3xl mx-auto print:hidden">
      <div className="mb-6 flex items-end justify-between">
        <div>
          <h2 className="text-sm font-bold text-indigo-600 uppercase tracking-wide mb-1">
            {topic} TEST
          </h2>
          ...
          {currentQ.options.map((option, idx) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-lg ${isSelected ? "text-indigo-900 font-medium" : "text-slate-700"}`}
                >
                  {option}
                </span>
              </button>
    ...
    <div className="animate-fade-in space-y-8 pb-12">
      ...
      <div className="grid md:grid-cols-3 gap-6 print:hidden">
        <div className="md:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            ...
            <p className="text-slate-500">
              You completed the{" "}
              <strong className="text-slate-700">{topic}</strong> assessment.
            </p>
            <div className="flex gap-4 justify-center md:justify-start mt-4">
              <div className="px-4 py-2 bg-green-50 rounded-lg border border-green-100 text-green-700 font-bold">
                {stats.correct} Correct
              </div>
              <div className="px-4 py-2 bg-red-50 rounded-lg border border-red-100 text-red-700 font-bold">
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl shadow-lg p-8 text-white flex flex-col justify-center items-center text-center">
          ...
          <button
            onClick={onRestart}
            className="w-full py-2 mb-3 bg-white text-indigo-600 font-bold rounded-xl hover:bg-indigo-50 transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="hidden print:block mb-8 border-b-2 border-slate-300 pb-4">
        <h1 className="text-3xl font-bold text-slate-900">Quiz.ai Report</h1>
        ...
        <div className="divide-y divide-slate-100 print:divide-slate-300">
          {questions.map((q, idx) => {
            ...
            return (
              <div
                key={q.id}
                className="p-6 hover:bg-slate-50 transition-colors print:px-0 print:hover:bg-white print:break-inside-avoid"
              >
                <div className="flex gap-4">
                  <div className="shrink-0 mt-1 print:hidden">
                    {isCorrect ? (
                      ...
                      <div className="w-6 h-6 font-bold text-slate-400">?</div>
                    ) : (
                  ...
                  <div className="space-y-3 flex-1">
                    ...
                    <div className="grid md:grid-cols-2 gap-4 text-sm print:grid-cols-1 print:gap-2">
                      <div
                        className={`p-3 rounded-lg border print:border-0 print:p-0 ${isCorrect ? "bg-green-50 border-green-200 text-green-800" : "bg-red-50 border-red-200 text-red-800"}`}
                      >
                        <span className="font-semibold block text-xs uppercase opacity-70 mb-1 print:inline print:mr-2">
                          Your Answer:
                        </span>
                      ...
                      <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 text-slate-800 print:bg-white print:border-0 print:p-0">
                        <span className="font-semibold block text-xs uppercase opacity-70 mb-1 text-slate-500 print:inline print:mr-2">
                          Correct Answer:
                        </span>
                        <span className="print:font-bold">
                          {q.correctAnswer}
        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 text-indigo-600">
            ...
            <span className="text-xl font-bold tracking-tight">
              Quiz<span className="text-slate-400">.ai</span>
            </span>
          </div>
          {appState !== "SELECTION" && (
            <button
          ...
          <div className="animate-fade-in space-y-8">
            <div className="text-center space-y-4 mb-12">
              ...
              <p className="text-lg text-slate-600 max-w-2xl mx-auto">
                Select your field of interest. It will generate a unique,
                20-question challenge tailored just for you.
              </p>
            </div>
            ...
            {!selectedDomain ? (
              <div className="grid md:grid-cols-2 gap-6">
                {DOMAINS.map((domain) => (
                  ...
                  >
                    <div
                      className={`absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity text-${domain.color}-600`}
                    >
                      {domain.icon}
                    </div>
                    <div
                      className={`inline-flex p-3 rounded-lg bg-${domain.color}-50 text-${domain.color}-600 mb-4`}
                    >
                      {domain.icon}
                    </div>
        ...
        {appState === "LOADING" && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center animate-fade-in">
            ...
            <p className="text-slate-500 mt-2 max-w-sm">
              AI is generating 20 unique MCQs for{" "}
              <span className="font-semibold text-indigo-600">
                {selectedSubDomain?.title}
              </span>
              ...
            </p>
          </div>
